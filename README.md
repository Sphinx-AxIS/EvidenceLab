...
If a Sigma draft returns zero matches, the manual tester now also runs deterministic probe variants to show whether the miss is caused by selectors such as `Channel`, `EventID`, `logsource.service`, or multi-token `|contains` clauses being too restrictive.
The Analytics page now also includes a `Rule Matches` tab. Analysts can select a Detection Rule from the current case/global rule inventory and run it against the current case on demand. Sigma rules reuse their compiled SQL path, while Suricata rules map to managed `suricata_alert` records by SID. The tab supports filters for rule title/type, matched record ID, timestamp, channel, and EventID/SID, and each row includes direct pivots to both Record Detail and Rule Review.

...
The Analytics page now also includes a `Rule Matches` tab. It surfaces deployed Sigma matches persisted during Windows-event ingest and managed Suricata signature hits correlated from `suricata_alert` records, with filters for rule title/type, matched record ID, timestamp, channel, and EventID/SID plus direct pivots to both Record Detail and Rule Review.
